...
function GetQueryStringValue(strElement, strQueryString) {
    ...
    if (foundValue === null) {
        WriteToDebug(
            "GetQueryStringValue Element '" + strElement + "' Not Found, Returning: empty string",
        );
        return '';
    } else {
    ...
    if (tzoffset != 0) {
        ...
        if (tzoffset > 0) {
            if (('' + tzoffset).indexOf('.') != -1) {
                var fraction =
                    '0' +
                    ('' + tzoffset).substr(('' + tzoffset).indexOf('.'), ('' + tzoffset).length);
...
function ConvertMilliSecondsIntoSCORM2004Time(intTotalMilliseconds) {
    WriteToDebug(
        'In ConvertMilliSecondsIntoSCORM2004Time intTotalMilliseconds=' + intTotalMilliseconds,
    );
    var ScormTime = '';
...
function SCORM2004_Initialize() {
    ...
    blnResult =
        SCORM2004_CallSetValue('cmi.exit', SCORM2004_TranslateExitTypeToSCORM(DEFAULT_EXIT_TYPE)) &&
        blnResult;
...
function SCORM2004_Finish(strExitType, blnStatusWasSet) {
    WriteToDebug(
        'In SCORM2004_Finish strExitType=' + strExitType + ', blnStatusWasSet=' + blnStatusWasSet,
    );
    ...
    if (strExitType == EXIT_TYPE_FINISH && !blnStatusWasSet) {
        ...
        blnResult =
            SCORM2004_CallSetValue('cmi.completion_status', strStatusAfterCompletion) && blnResult;
    }
    ...
    blnResult =
        SCORM2004_CallSetValue('cmi.exit', SCORM2004_TranslateExitTypeToSCORM(strExitType)) &&
        blnResult;
...
function SCORM2004_RecordMatchingInteraction(
    ...
    for (var i = 0; i < aryCorrectResponse.length; i++) {
        ...
        strCorrectResponse +=
            aryCorrectResponse[i].Source.Long + '[.]' + aryCorrectResponse[i].Target.Long;
    }
    return SCORM2004_RecordInteraction(
        strID,
        strResponse,
...
function SCORM2004_SetObjectiveScore(strObjectiveID, intScore, intMaxScore, intMinScore) {
    ...
    blnResult =
        SCORM2004_CallSetValue('cmi.objectives.' + intObjectiveIndex + '.score.raw', intScore) &&
        blnResult;
    blnResult =
        SCORM2004_CallSetValue('cmi.objectives.' + intObjectiveIndex + '.score.max', intMaxScore) &&
        blnResult;
    blnResult =
        SCORM2004_CallSetValue('cmi.objectives.' + intObjectiveIndex + '.score.min', intMinScore) &&
        blnResult;
    blnResult =
        SCORM2004_CallSetValue(
...
function SCORM2004_GetInteractionTimestamp(strInteractionID) {
    ...
    return SCORM2004_CallGetValue(
        ConvertIso8601TimeStampToDate('cmi.interactions.' + intInteractionIndex + '.timestamp'),
    );
}
function SCORM2004_GetInteractionCorrectResponses(strInteractionID) {
    WriteToDebug(
        'In SCORM2004_GetInteractionCorrectResponses, strInteractionID=' + strInteractionID,
    );
    var intInteractionIndex = SCORM2004_FindInteractionIndexFromID(strInteractionID);
    WriteToDebug('intInteractionIndex=' + intInteractionIndex);
    SCORM2004_ClearErrorInfo();
...
function SCORM2004_GetInteractionLearnerResponses(strInteractionID) {
    WriteToDebug(
        'In SCORM2004_GetInteractionLearnerResponses, strInteractionID=' + strInteractionID,
    );
    var intInteractionIndex = SCORM2004_FindInteractionIndexFromID(strInteractionID);
    WriteToDebug('intInteractionIndex=' + intInteractionIndex);
    SCORM2004_ClearErrorInfo();
    ...
    var strType = SCORM2004_CallGetValue('cmi.interactions.' + intInteractionIndex + '.type');
    var strResponse = new String(
        SCORM2004_CallGetValue('cmi.interactions.' + intInteractionIndex + '.learner_response'),
    );
...
function SCORM_Finish(strExitType, blnStatusWasSet) {
    ...
    var blnResult = true;
    WriteToDebug(
        'In SCORM_Finish strExitType=' + strExitType + ', blnStatusWasSet=' + blnStatusWasSet,
    );
    SCORM_ClearErrorInfo();
    if (strExitType == EXIT_TYPE_FINISH && !blnStatusWasSet) {
        WriteToDebug('Getting completion status');
        ...
        WriteToDebug('Setting completion status to ' + strStatusAfterCompletion);
        blnResult =
            SCORM_CallLMSSetValue('cmi.core.lesson_status', strStatusAfterCompletion) && blnResult;
    }
    WriteToDebug('Setting Exit');
    blnResult =
...
function SCORM_RecordMatchingInteraction(
    ...
    for (var i = 0; i < aryCorrectResponse.length; i++) {
        ...
        strCorrectResponse +=
            aryCorrectResponse[i].Source.Short + '.' + aryCorrectResponse[i].Target.Short;
        strCorrectResponseLong +=
            aryCorrectResponse[i].Source.Long + '.' + aryCorrectResponse[i].Target.Long;
    }
    var blnSuccessfullySaved;
...
function SCORM_SetObjectiveScore(strObjectiveID, intScore, intMaxScore, intMinScore) {
    ...
    blnResult =
        SCORM_CallLMSSetValue('cmi.objectives.' + intObjectiveIndex + '.score.raw', intScore) &&
        blnResult;
    blnResult =
        SCORM_CallLMSSetValue('cmi.objectives.' + intObjectiveIndex + '.score.max', intMaxScore) &&
        blnResult;
    blnResult =
        SCORM_CallLMSSetValue('cmi.objectives.' + intObjectiveIndex + '.score.min', intMinScore) &&
        blnResult;
    WriteToDebug('Returning ' + blnResult);
    return blnResult;
}
function SCORM_SetObjectiveDescription(strObjectiveID, strObjectiveDescription) {
    ...
    var blnResult;
    WriteToDebug(
        'In SCORM_SetObjectiveDescription, strObjectiveDescription=' + strObjectiveDescription,
    );
...
function SCORM_SetObjectiveStatus(strObjectiveID, Lesson_Status) {
    ...
    blnResult =
        SCORM_CallLMSSetValue('cmi.objectives.' + intObjectiveIndex + '.status', strSCORMStatus) &&
        blnResult;
    WriteToDebug('Returning ' + blnResult);
    return blnResult;
}
...
function SCORM_GetLastErrorDesc() {
    WriteToDebug(
        'In SCORM_GetLastErrorDesc, ' + strSCORMErrorString + '\n' + strSCORMErrorDiagnostic,
    );
...
function AICC_Finish(strExitType, blnStatusWasSet) {
    WriteToDebug(
        'In AICC_Finish, strExitType=' + strExitType + ', blnStatusWasSet=' + blnStatusWasSet,
    );
    if (!blnStatusWasSet) {
...
function AICC_SetError(intErrorNum, strErrorDesc) {
    WriteToDebug(
        'ERROR: In AICC_SetError, intErrorNum=' + intErrorNum + ', strErrorDesc=' + strErrorDesc,
    );
...
function AICC_TranslateTimeLimitAction(strTimeLimitAction) {
    ...
    if (arySplit.length == 2) {
        ...
        if (
            (strChar1 != 'e' && strChar1 != 'c' && strChar1 != 'm' && strChar1 != 'n') ||
    ...
    for (i = 0; i < aryLines.length; i++) {
        ...
        if (strLineType == 'j_id') {
            ...
            AICC_aryObjectivesRead[parseInt(strIndex, 10)][AICC_OBJ_ARRAY_STATUS] =
                AICC_ConvertAICCStatusIntoLocalStatus(strLineValue.charAt(0).toLowerCase());
...
function AICC_TranslateObjectivesToAICC() {
    ...
    for (var i = 0; i < AICC_aryObjectivesWrite.length; i++) {
        ...
        if (AICC_aryObjectivesWrite[i]) {
            ...
            strObjectives +=
                'J_ID.' + (i + 1) + '=' + AICC_aryObjectivesWrite[i][AICC_OBJ_ARRAY_ID] + '\r\n';
    ...
    LRS.prototype = {
        ...
        init: function (cfg) {
            ...
            if (cfg.hasOwnProperty('auth')) {
                ...
                this.auth =
                    'Basic ' + TinCan.Utils.getBase64String(cfg.username + ':' + cfg.password);
            }
        ...
        saveStatement: function (stmt, cfg) {
            ...
            cfg = cfg || {};
            requestCfg = { url: 'statements', data: JSON.stringify(stmt.asVersion(this.version)) };
            if (stmt.id !== null) {
...
function TCAPI_Initialize() {
    ...
    WriteToDebug(
        'TCAPI_Initialize - fetching cumulative time from state: ' + TCAPI_STATE_TOTAL_TIME,
    );
    ...
    tincan.sendStatement(
        ...
        function (results, statement) {
            ...
            WriteToDebug(
                'TCAPI_Initialize - record initial launch statement success: ' + statement.id,
            );
        },
    );
    InitializeExecuted(true, '');
    ...
    tincan.sendStatement(
        {
            ...
            object: {
                ...
                definition: { name: { 'en-US': name !== undefined && name !== '' ? name : value } },
            },
...
function TCAPI_GetPreviouslyAccumulatedTime() {
    ...
    var data = 0,
        getStateResult;
    WriteToDebug(
        'In TCAPI_GetPreviouslyAccumulatedTime - cached: ' + tcapi_cache.totalPrevDuration,
    );
...
function TCAPI_SetObjectiveDescription(strObjectiveID, strObjectiveDescription) {
    WriteToDebug(
        'In TCAPI_SetObjectiveDescription, strObjectiveDescription=' + strObjectiveDescription,
    );
    TCAPI_ClearErrorInfo();
    return TCAPI_TRUE;
}
...
function TCAPI_GetLastErrorDesc() {
    WriteToDebug(
        'In TCAPI_GetLastErrorDesc, ' + strTCAPIErrorString + '\n' + strTCAPIErrorDiagnostic,
    );
...
function LMSStandardAPI(strStandard) {
    ...
    eval(
        'this.GetInteractionCorrectResponses = ' + strStandard + '_GetInteractionCorrectResponses',
    );
    eval('this.GetInteractionWeighting = ' + strStandard + '_GetInteractionWeighting');
    eval(
        'this.GetInteractionLearnerResponses = ' + strStandard + '_GetInteractionLearnerResponses',
    );
...
function Start() {
    ...
    WriteToDebug('----------------------------------------');
    WriteToDebug(
        'In Start - Version: ' + VERSION + '  Last Modified=' + window.document.lastModified,
    );
    ...
    WriteToDebug(
        'strStandAlone=' + strStandAlone + '  strShowInteractiveDebug=' + strShowInteractiveDebug,
    );
...
function InitializeExecuted(blnSuccess, strErrorMessage) {
    WriteToDebug(
        'In InitializeExecuted, blnSuccess=' + blnSuccess + ', strErrorMessage=' + strErrorMessage,
    );
    if (!blnSuccess) {
...
function SetTextPreference(intPreference) {
    ...
    if (
        ...
        SetErrorInfo(
            ERROR_INVALID_PREFERENCE,
            'Invalid Preference passed to SetTextPreference, intPreference=' + intPreference,
        );
        return false;
    }
...
function SetScore(intScore, intMaxScore, intMinScore) {
    ...
    if (!IsValidDecimal(intScore)) {
        ...
        SetErrorInfo(
            ERROR_INVALID_NUMBER,
            'Invalid Score passed to SetScore (not a valid decimal), intScore=' + intScore,
        );
        return false;
    }
    ...
    if (strLMSStandard == 'SCORM') {
        ...
        if (intScore < 0 || intScore > 100) {
            ...
            SetErrorInfo(
                ERROR_INVALID_NUMBER,
                'Invalid Score passed to SetScore (must be between 0-100), intScore=' + intScore,
            );
            return false;
        }
...
function SetObjectiveScore(strObjectiveID, intScore, intMaxScore, intMinScore) {
    ...
    if (!IsValidDecimal(intScore)) {
        ...
        SetErrorInfo(
            ERROR_INVALID_NUMBER,
            'Invalid Score passed to SetObjectiveScore (not a valid decimal), intScore=' + intScore,
        );
        return false;
    }
    if (!IsValidDecimal(intMaxScore)) {
...
function SetObjectiveStatus(strObjectiveID, Lesson_Status) {
    ...
    if (
        ...
        SetErrorInfo(
            ...
            'Invalid status passed to SetObjectiveStatus, Lesson_Status=' + Lesson_Status,
        );
        return false;
    }
    WriteToDebug('Calling To LMS');
